...
class CartItem extends HTMLElement {
  ...
  set quantity(val) {
    this.setAttribute('quantity', val);
    // Instead of full render on quantity set, update input value only
    const input = this.shadowRoot.querySelector('input[type=number]');
  ...
  attachListeners() {
    const qtyInput = this.shadowRoot.querySelector('input[type=number]');
    const removeBtn = this.shadowRoot.querySelector('.remove-btn');

    if (qtyInput) {
      qtyInput.addEventListener('change', this.onQuantityChange);
    }
    if (removeBtn) {
  ...
  detachListeners() {
    ...
    if (qtyInput) {
      qtyInput.removeEventListener('change', this.onQuantityChange);
    }
    if (removeBtn) {
  ...
  render() {
    ...
    // Detach listeners before replacing DOM
    this.detachListeners();
